...
// 1) Expose active STAR procedures with public workspace config.
replace(
...
// 2) App workspace config carries STAR procedure mappings and passes current runway mappings to panel.
replace(
  'src/App.tsx',
...
// 3) IVAO panel recognizes filed STAR designator and prioritizes its configured entry fix.
replace(
...
replace(
  ...
  `    autoAssignedFix = false,\n    filedStarDesignator: string | null = null,\n  ) => {\n    let estimate = { ...autoEstimate(flight, geometry, refFix, groundSpeed, baseTimeIso, lookaheadMin), filedStarDesignator }`
);
replace(
  'src/IvaoTrafficPanel.tsx',
...
// Labels: a STAR designator in FPL is not an unfiled-fix case.
replace(
  'src/IvaoTrafficPanel.tsx',
...
replace(
  ...
  `      if (estimate.assumedDirect && estimate.filedStarDesignator) return 'MANUAL ETO · filed STAR ' + estimate.filedStarDesignator + ' entry ' + estimate.refFix + ' ' + estimate.eto + 'Z available' + domesticEta\n      if (estimate.assumedDirect) return 'MANUAL ETO · assumed-DCT auto estimate ' + estimate.refFix + ' ' + estimate.eto + 'Z available' + domesticEta`
);
replace(
  'src/IvaoTrafficPanel.tsx',
  ...
  `    if (estimate.assumedDirect) {\n      const past = estimate.pastCrossing ? ' · EST PAST XING' : ''\n      const routeLabel = estimate.filedStarDesignator ? filedStar + ' · ROUTE→STAR ENTRY' : ' · REF FIX NOT FILED · ASSUMED DCT'\n      return 'AUTO ETO · ' + estimate.refFix + ' ~' + estimate.eto + 'Z' + routeLabel + past + ' · ' + Math.round(estimate.remainingNm || 0) + ' NM · GS ' + Math.round(estimate.groundSpeed || 0) + domesticEta\n    }`
);
replace(
  'src/IvaoTrafficPanel.tsx',
  `      const assumed = estimate.assumedDirect ? ' · REF FIX NOT FILED · ASSUMED DCT' : ''`,
  `      const assumed = estimate.assumedDirect ? (estimate.filedStarDesignator ? filedStar + ' · ROUTE→STAR ENTRY' : ' · REF FIX NOT FILED · ASSUMED DCT') : ''`
);
replace(
  'src/IvaoTrafficPanel.tsx',
...
// Remove temporary patch files in resulting commit.
